...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-[720px] mx-auto px-4 pb-8">
      ...
      <Card className="mt-6 mb-5">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {user.company && <p className="text-[12px] text-slate-400">{user.company}</p>}
          </div>
          <Button size="sm" variant="outline">편집</Button>
        </div>
        <div className="flex gap-4 pt-3 border-t border-slate-100">
          <div className="text-center flex-1">
            <p className="text-[20px] font-bold text-blue-600 tabular-nums">{activeSiteCount}</p>
            <p className="text-[11px] text-slate-400 mt-0.5">담당현장</p>
          </div>
          <div className="text-center flex-1">
            <p className="text-[20px] font-bold text-ink tabular-nums">{totalEntries}</p>
            <p className="text-[11px] text-slate-400 mt-0.5">누적입력</p>
          </div>
          {user.joined && (
            <div className="text-center flex-1">
              <p className="text-[14px] font-bold text-slate-500">{user.joined}</p>
              <p className="text-[11px] text-slate-400 mt-0.5">가입</p>
            </div>
          )}
        </div>
      </Card>
      ...
      {/* Account */}
      <p className="text-[12px] font-semibold text-slate-400 px-1 mb-2">계정</p>
        ...
        {[
          ...
          <div key={label} className="flex items-center justify-between px-4 py-3.5">
            ...
            <div className="flex items-center gap-1">
              {value && <span className="text-[13px] text-slate-400">{value}</span>}
      ...
      {/* Notifications */}
      <p className="text-[12px] font-semibold text-slate-400 px-1 mb-2">알림</p>
      <Card className="mb-5 divide-y divide-slate-100 !p-0">
        {[
